...
/// Normalize a DAT1 asset path: lowercase, forward slashes, collapse consecutive
/// slashes, strip leading/trailing slashes.
pub fn normalize_path(path: &str) -> String {
    let lowered = path.to_lowercase();
    let with_fwd_slashes = lowered.replace('\\', "/");

    let mut result = String::with_capacity(with_fwd_slashes.len());
    let mut prev_slash = false;
    for ch in with_fwd_slashes.chars() {
        if ch == '/' {
            if !prev_slash {
                result.push(ch);
            }
...
pub fn hash_raw(data: &[u8]) -> u64 {
    ...
    for &byte in data {
        crc = (crc >> 8) ^ TABLE[(0xFF & (crc ^ byte as u64)) as usize];
    }
    (crc >> 2) | 0x8000000000000000
}

/// Normalize the path and compute its DAT1 CRC64 hash.
pub fn hash(data: &str) -> u64 {
    let normalized = normalize_path(data);
    log::debug!("crc64::hash: normalized {:?} -> {:?}", data, normalized);
    let value = hash_raw(normalized.as_bytes());
    log::debug!("crc64::hash: result = {:#018X}", value);
    value
}
...
mod tests {
    ...
    fn hash_known_values() {
        ...
        let empty_hash = hash("");
        assert_eq!(empty_hash, (0xC96C5795D7870F42_u64 >> 2) | 0x8000000000000000);

        // Single character 'a'
        let a_hash = hash("a");
        let expected_a = {
            let mut crc: u64 = 0xC96C5795D7870F42;
            crc = (crc >> 8) ^ TABLE[(0xFF & (crc ^ b'a' as u64)) as usize];
            (crc >> 2) | 0x8000000000000000
        };
    ...
    fn hash_is_case_insensitive() {
        assert_eq!(hash("Foo/Bar"), hash("foo/bar"));
        assert_eq!(hash("FOO\\BAR"), hash("foo/bar"));
    }

    #[test]
    fn hash_normalizes_slashes() {
    ...
    fn hash_raw_deterministic() {
        ...
        assert_eq!(hash_raw(data), hash_raw(data));
    }
}
